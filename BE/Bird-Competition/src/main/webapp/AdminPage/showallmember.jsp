                ...
                <div class="row d-flex justify-content-center align-items-center h-100">
                    ...
                    <div class="col col-lg-9 col-xl-7">
                        <div class="card">
                            ...
                            <c:set var="memberList" value="${sessionScope.OWN_MEMBER}"/>
                            <c:if test="${not empty memberList}">                   
                                <c:forEach items="${memberList}" var="p" varStatus="loop"> 
                                            ...
                                            <div class="box col mb-5">
                                                <div class="imghover">
                                                    <form action="DispatchServlet">
                                                        <input type="hidden" name="txtMemberID" value="${p.memberID}" />
                                                        <button class="btn" name="btAction" value="MemberProfile">Thông Tin</button>
                                                        
                                                    </form>
                                                </div>
                                                <img src="FE/img/${p.photoPath}">
                                                <p class="birdname" style="font-weight: bold; text-align:center;">${p.memberID}</p>
                                                
                                            </div>
                                            <c:if test="${nextIndex <= memberList.size() - 1}">

                                                <div class="box col mb-5 ">
                                                    <div class="imghover">
                                                        <form action="DispatchServlet">
                                                            <input type="hidden" name="txtMemberID" value="${memberList[nextIndex].memberID}" />
                                                            <button class="btn" name="btAction" value="MemberProfile">Thông Tin</button>
                                                        
                                                            
                                                        </form>
                                                    </div>
                                                    <img src="FE/img/${memberList[nextIndex].photoPath}">
                                                    <p class="birdname" style="font-weight: bold; text-align:center;">${memberList[nextIndex].birdName}
                                                    </p>
                                                </div>
                                            </c:if>
                                            <c:if test="${nextIndex > memberList.size() - 1}">
